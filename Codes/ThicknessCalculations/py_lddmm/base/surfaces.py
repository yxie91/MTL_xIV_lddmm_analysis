...
from netgen.csg import CSGeometry, Cylinder, Sphere as Sphere_ng, Plane as Plane_ng, Pnt, Vec, OrthoBrick
...
class Surface:
    def __init__(self, surf=None, weights=None, checkOrientation=False):
        if type(surf) in (list, tuple):
            if isinstance(surf[0], Surface):
                ...
                self.computeCentersAreas(checkOrientation=checkOrientation)
        elif type(surf) is str:
            self.read(surf, checkOrientation=checkOrientation)
            ...
            if weights is None:
                self.vertex_weights = np.copy(surf.vertex_weights)
    ...
    def read(self, filename, checkOrientation=False):
        (mainPart, ext) = os.path.splitext(filename)
        if ext in ('.byu', '.g'):
            self.readbyu(filename, checkOrientation=checkOrientation)
        elif ext == '.off':
            self.readOFF(filename, checkOrientation=checkOrientation)
        elif ext == '.vtk':
            self.readVTK(filename, checkOrientation=checkOrientation)
        elif ext == '.stl':
            self.readSTL(filename, checkOrientation=checkOrientation)
        elif ext == '.obj':
            self.readOBJ(filename, checkOrientation=checkOrientation)
        else:
            self.vertices = np.empty(0)
            self.centers = np.empty(0)
            self.faces = np.empty(0)
    ...
    def from_netgen(self, mesh, targetSize=None):
        p_ = mesh.ngmesh.Points()
        m = mesh.ngmesh.Elements2D().NumPy()
        p = np.zeros((len(p_), 3))
        for k, pt in enumerate(p_):
            p[k, :] = pt.p
        #f = m['nodes'][:,:3]-1
        self.vertices = p
        self.faces = m['nodes'][:,:3]-1
        self.removeIsolated()
        self.component = np.zeros(self.faces.shape[0], dtype=int)
    ...
    def from_pv(self, mesh, targetSize=None):
    ...
    def computeCentersAreas(self, checkOrientation=False):
        xDef1 = self.vertices[self.faces[:, 0], :]
        xDef2 = self.vertices[self.faces[:, 1], :]
        xDef3 = self.vertices[self.faces[:, 2], :]
        areas = np.cross(xDef2-xDef1, xDef3-xDef1)/2
        if checkOrientation:
            if areas.min() < -1e-12:
                if areas.max() > 1e-12:
                    print('Warning: mesh has inconsistent orientation',
                          (areas < 0).sum(), (areas > 0).sum())
                else:
                    f_ = np.copy(self.faces[:, 1])
                    self.faces[:, 1] = np.copy(self.faces[:, 2])
                    self.faces[:, 2] = f_
                    xDef2 = self.vertices[self.faces[:, 1], :]
                    xDef3 = self.vertices[self.faces[:, 2], :]
                    areas = np.cross(xDef2 - xDef1, xDef3 - xDef1) / 2
        ...
        w1 = self.vertex_weights[self.faces[:, 0]]
        w2 = self.vertex_weights[self.faces[:, 1]]
        w3 = self.vertex_weights[self.faces[:, 2]]
        self.face_weights = (w1+w2+w3)/3

    def updateWeights(self, w0):
        self.vertex_weights = np.copy(w0)
        w1 = self.vertex_weights[self.faces[:, 0]]
        w2 = self.vertex_weights[self.faces[:, 1]]
        w3 = self.vertex_weights[self.faces[:, 2]]
        self.face_weights = (w1+w2+w3)/3
    ...
    def computeVertexNormals(self):
        ...
        af = np.maximum(1e-10, np.sqrt((normals**2).sum(axis=1)))
        # logging.info(f'min area = {af.min():.8f} max area: {af.max():.4f}')
    ...
    def getEdges(self):
        (self.edges, self.edgeFaces,
         self.faceEdges, self.bdry) = get_edges_(self.faces)
    ...
    def LocalSignedDistance(self, data, value):
        ...
        u[rad + 1:n0 - rad, rad + 1:n1 - rad, rad + 1:n2 - rad] = \
            (c2[diam:n0, diam:n1, diam:n2]
        ...
        J0 = np.nonzero(np.fabs(u) < maxval)
        # print len(I[0])

        for k in range(len(J0[0])):
            p = np.array((J0[0][k], J0[1][k], J0[2][k]))
            bmin = p - rad
            bmax = p + rad + 1
            # print p, bmin, bmax
            if (d2[p[0], p[1], p[2]] > 0):
                res[p[0], p[1], p[2]] = min(cube[np.nonzero(d2[bmin[0]:bmax[0],
                                                               bmin[1]:bmax[1],
                                                               bmin[2]:bmax[2]] < 0)]) - .25
            else:
                res[p[0], p[1], p[2]] = - min(cube[np.nonzero(d2[bmin[0]:bmax[0],
    ...
    def Isosurface_ski(self, data, value=0.5, step=1):
        verts, faces, n, v = measure.marching_cubes(data,
    ...
    def edgeRecover(self):
        ...
        while lastOriented < len(activeList)-1:
            ...
            I0 = np.nonzero(edgF[j, :])
            foundOne = False
            for kk in I0[0].tolist():
                if (foundOne is False) and (isOriented[kk]):
                    foundOne = True
                    u1 = edgF[j, kk] - 1
                    u2 = edgF[kk, j] - 1
    ...
    def removeIsolated(self):
        ...
        logging.info(f'Found {N-J[0].shape[0]} isolated vertices')
        Q = -np.ones(N)
    ...
    def removeDuplicates(self, c=0.0001, verb=False):
        ...
        for kj in range(1, N0):
            ...
            if (len(J) > 0):
                ...
                newweights[N] = self.vertex_weights[kj]
                N = N + 1

        newv = newv[0:N, :]
        newweights = newweights[0:N]
        self.vertices = newv
        self.vertex_weights = newweights
    ...
    def split_in_4(self, faces, vert, weights, val, popped, edges, i,
                   with_vertex=None):
        if not popped[i]:
    ...
    def truncate(self, ineq=(), val=None):
        ...
        weights = list(res.vertex_weights)
        edges = {}
    ...
    def cut(self, select):
        ...
        res.updateWeights(self.vertex_weights[select])
    ...
    def createFlatApproximation(self, thickness=None, M=50):
        ...
        geo = CSGeometry()
        s1 = Cylinder(Pnt(0,1,1), Pnt(2,1,1), np.sqrt(3))
        c1 = OrthoBrick(Pnt(0,-2,-2), Pnt(2,4,4))
        # s1 = Sphere_ng(Pnt(1., 1., 1.), np.sqrt(3))
        geo.Add(s1*c1)
        # mesh = Mesh(geo.GenerateMesh(maxh=0.02, perfstepsend=MeshingStep.MESHSURFACE))
        mesh = Mesh(geo.GenerateMesh(maxh=0.2))
        h = Surface()
        h.from_netgen(mesh)
        h.vertices[:, 0] = 1 + (h.vertices[:, 0] - 1) * np.sqrt(w0)/emax
        h.vertices[:, 1] = 1 + (h.vertices[:, 1] - 1) * np.sqrt(w[1])/emax
        h.vertices[:, 2] = 1 + (h.vertices[:, 2] - 1) * np.sqrt(w[2])/emax
        h.updateVertices(h.vertices)
        ...
        #         y = emax * (x0[1] - 1)
        #         z = emax * (x0[2] - 1)
        #         I1 = np.maximum(z ** 2 / w[2] + y ** 2 / w[1] + x ** 2/w0 - 3,
        #                         x ** 2/w0 - 1)
        #         return I1
        #
        ...
        #
        # d = Pancake()
        # mesh = generate_surface_mesh(d, max_facet_distance=1.0,
    ...
    def rayTraces(self, points, ray):
        ...
        det = c00*c11*c22 - c00*c12*c21 - c01*c10*c22 - c02*c11*c20\
            + c01*c12*c20 + c02*c10*c21
    ...
    def plot(self, fig=1, ec='b', fc='r', al=.5, lw=1, azim=100, elev=45,
             setLim=True, addTo=False):
        f = plt.figure(fig)
        plt.clf()
    ...
    # Reads from .off file
    def readOFF(self, offfile, checkOrientation=False):
    ...
    # Reads from .byu file
    def readbyu(self, byufile, checkOrientation=False):
    ...
    def saveVTK(self, fileName, scalars=None, normals=None, tensors=None,
                scal_name='scalars', vectors=None, vect_name='vectors'):
        vf = vtkFields('POINT_DATA', self.vertices.shape[0])
        vf.scalars['weights'] = self.vertex_weights
        if not (scalars is None):
            vf.scalars[scal_name] = scalars
        if not (vectors is None):
            vf.vectors[vect_name] = vectors
        if not (normals is None):
            vf.normals['normals'] = normals
        if not (tensors is None):
            vf.tensors['tensors'] = tensors
        self.saveVTK2(fileName, vf)

    # Saves in .vtk format
    def saveVTK2(self, fileName, vtk_fields=None):
        F = self.faces
        V = self.vertices
        ...
        with open(fileName, 'w') as fvtkout:
            ...
            if vtk_fields is not None:
                ...
                #     for k in range(nf):
                #         fvtkout.write('\nSCALARS '+ vtkFields.scalars[2*k] +' float\nLOOKUP_TABLE default')
                #         # print('vtk', vtkFields.scalars[2 * k], vtkFields.scalars[2 * k+1].shape, vtkFields.scalars[2 * k+1].min(),
                #         #        vtkFields.scalars[2 * k+1].max())
                #         # fvtkout.write('\nSCALARS '+ vtkFields.scalars[2*k] +' float 1\nLOOKUP_TABLE default')
                #         for ll in range(V.shape[0]):
                #             # print(vtkFields.scalars[2*k+1][ll])
    ...
    # Reads .vtk file
    def readVTK(self, fileName, checkOrientation=False, withPointData=None,
                withCellData=None, withFieldData=None):
        if gotVTK:
            u = vtkPolyDataReader()
            u.ReadAllVectorsOn()
            u.ReadAllScalarsOn()
            ...
            w0 = v.GetPointData()
            c0 = v.GetCellData()
            lab = c0.GetScalars('labels')
            f0 = v.GetFieldData()
            output = {}
            if withPointData is not None:
                output['pointData'] = {}
                for idx in range(w0.GetNumberOfArrays()):
                    array = w0.GetArray(idx)
                    name = array.GetName()
                    if name in withPointData:
                        output['pointData'][name] = vtk_to_numpy(array)
            if withCellData is not None:
                output['cellData'] = {}
                for idx in range(c0.GetNumberOfArrays()):
                    array = c0.GetArray(idx)
                    name = array.GetName()
                    print(name)
                    if name in withCellData:
                        output['cellData'][name] = vtk_to_numpy(array)
            if withFieldData is not None:
                output['fieldData'] = {}
                print(f0.GetNumberOfArrays())
                for idx in range(f0.GetNumberOfArrays()):
                    array = f0.GetArray(idx)
                    name = array.GetArrayName(idx)
                    print(name)
                    if name in withCellData:
                        output['fieldData'][name] = vtk_to_numpy(array)
    ...
    def readFromImage(self, fileName, with_vfld=True):
        ...
        if with_vfld:
            smoothData = sp.sparse.linalg.cg(lambda x: -diffeo.laplacian(x),
                                             -self.img.data, maxiter=500)
    ...
    # Reads .obj file
    def readOBJ(self, fileName, checkOrientation=False):
        if gotVTK:
            u = vtkOBJReader()
            u.SetFileName(fileName)
            u.Update()
    ...
    # Reads .stl file
    def readSTL(self, fileName, checkOrientation=False):
        if gotVTK:
            u = vtkSTLReader()
            u.SetFileName(fileName)
            u.Update()
    ...
    def connected_components(self, split=False):
        self.getEdges()
        N = self.edges.max()+1
        A = csr_matrix((np.ones(self.edges.shape[0]),
    ...
    def extract_components(self, comp=None, comp_info=None):
        if comp_info is not None:
            ...
            if self.edges is None:
                F, J, E, FE, EF \
                    = extract_components_(comp, self.vertices.shape[0],
                                          self.faces, self.component,
                                          edge_info=None)
            else:
                F, J, E, FE, EF \
                    = extract_components_(comp, self.vertices.shape[0],
    ...
    def normGrad(self, phi):
        ...
        u = l1*((phi2-phi1)*(phi3-phi1)).sum(axis=1) \
            + l2*((phi3-phi2)*(phi1-phi2)).sum(axis=1)\
    ...
    def diffNormGrad(self, phi):
        ...
        r1 = - u * np.cross(v2-v3, self.surfel)\
            + 2*((v1-v3)*(((phi3-phi2)*(phi1-phi2)).sum(axis=1))[:, np.newaxis]
                 + (v1-v2)*(((phi1-phi3)*(phi2-phi3)).sum(axis=1)[:, np.newaxis]))/a
        r2 = - u * np.cross(v3-v1, self.surfel)\
            + 2*((v2-v1)*(((phi1-phi3)*(phi2-phi3)).sum(axis=1))[:, np.newaxis]
                 + (v2-v3)*(((phi2-phi1)*(phi3-phi1)).sum(axis=1))[:, np.newaxis])/a
        r3 = - u * np.cross(v1-v2, self.surfel)\
            + 2*((v3-v2)*(((phi2-phi1)*(phi3-phi1)).sum(axis=1))[:, np.newaxis]
