...
class vtkFields:
    def __init__(self, data_type='', size=0, scalars=None, vectors=None,
                 normals=None, tensors=None, fields=None):
        if scalars is None:
            self.scalars = dict()
        else:
        ...
        if len(self.fields):
            ...
            for n in names:
                ...
                if isinstance(a, dict):
                    ...
                    for nm in a.keys():
                        tp = _get_type(a[nm])
                        fvtkout.write(f'\n{nm} {a[nm].shape[1]} {a[nm].shape[0]} {tp}')
                        for kk in range(a[nm].shape[0]):
                            fvtkout.write('\n')
                            for ll in range(a[nm].shape[1]):
                                fvtkout.write(f'{a[nm][kk, ll]} ')
                else:
                    fvtkout.write(f'\nFIELD field {1}')
                    tp = _get_type(a)
