...
class GaussianDiffeons(PointSet):
    def __init__(self, data=None, method = None, rate = 1., decimationTarget = None):
        super().__init__()
        self.tensors = None
        self.labels = None
        if data is not None:
            if type(data) in (list, tuple):
                ...
                self.tensors = data[1]
                if len(data) ==2:
                    self.labels = np.ones(self.vertices.shape[0], dtype=int)
                else:
                    self.labels = data[2]
            else:
                if method == 'segmentation':
                    self.vertices, self.tensors, self.labels = generateDiffeonsFromSegmentation(data, rate)
                elif method == 'net':
                    self.vertices, self.tensors, self.labels = generateDiffeonsFromNet(data, rate)
                else:
                    if decimationTarget is None:
                        decimationTarget = int(data.vertices.shape[0] * rate)
                    self.vertices, self.tensors, self.labels = generateDiffeonsFromDecimation(data, decimationTarget)

    def saveVTK(self, fileName):
        saveDiffeons(fileName, self.vertices, self.tensors)
