...
class KernelSpec:
    def __init__(self, name='gauss', affine = 'none', sigma = (1.,), order = 3, w1 = 1.0,
                 w2 = 1.0, dim = 3, weight = None, localMaps=None, KP = -1):
        self.name = name
        if np.isscalar(sigma):
            ...
            self.sigma = np.array(sigma, dtype=float)
        if weight is None:
            self.weight = self.sigma ** KP
            self.weight /= self.weight.sum()
        else:
            if np.isscalar(weight):
                self.weight = np.array([weight]*len(self.sigma), dtype=float)
            else:
...
class Kernel(KernelSpec):
    ...
    def applyK(self, x, a, firstVar = None, grid=None,matrixWeights=False, cpu=False):
        if firstVar is None:
            y = np.copy(x)
            if matrixWeights:
                z = ku.applykmat(y, x, a, self.name, self.sigma, self.order, self.weight)
            elif self.localMaps:
                if self.localMaps[2] == 'naive':
                    z = ku.applylocalk_naive(y ,x ,a ,self.name, self.sigma ,self.order, self.weight)
                else:
                    z = ku.applylocalk(y ,x ,a ,self.name, self.sigma ,self.order, self.weight, self.localMaps[0] ,
                                        self.localMaps[1], KP=self.ms_exponent)
            else:
                z = ku.applyK(y ,x ,a ,self.name, self.sigma ,self.order, self.weight, cpu=cpu, dtype=self.pk_dtype)
        else:
            if matrixWeights:
                z = ku.applykmat(firstVar, x, a, self.name, self.sigma, self.order, self.weight)
            elif self.localMaps:
                if self.localMaps[2] == 'naive':
                    z = ku.applylocalk_naive(firstVar ,x ,a ,self.name, self.sigma ,self.order, self.weight)
                else:
                    z = ku.applylocalk(firstVar ,x ,a ,self.name, self.sigma ,self.order, self.weight, self.localMaps[0] ,
                                        self.localMaps[1])
            else:
                z = ku.applyK(firstVar ,x ,a , self.name, self.sigma ,self.order, self.weight, cpu=cpu, dtype=self.pk_dtype)
    ...
    def applyKTensor(self, x, ay, ax, betay, betax, firstVar = None, grid=None, cpu=False):
        if firstVar is None:
            y = np.copy(x)
            z = ku.applyktensor(y ,x , ay, ax, betay, betax ,self.name, self.sigma ,self.order, self.weight, cpu=cpu,
                                dtype=self.pk_dtype)
        else:
            z = ku.applyktensor(firstVar ,x , ay, ax, betay, betax , self.name, self.sigma ,self.order, self.weight, cpu=cpu,
                                dtype=self.pk_dtype)
        return z

    # # Computes A(i) = sum_j D_1[K(x(i), x(j))a2(j)]a1(i)
    def applyDiffK(self, x, a1, a2):
        z = ku.applykdiff1(x, a1, a2, self.name, self.sigma, self.order, self.weight)
        if self.affine == 'affine':
            xx = x - self.center
    ...
    def applyDiffK2(self, x, a1, a2):
        z = ku.applykdiff2(x, a1, a2, self.name, self.sigma, self.order, self.weight)
        if self.affine == 'affine':
            xx = x - self.center
    ...
    def applyDiffK1and2(self, x, a1, a2):
        z = ku.applykdiff1and2(x, a1, a2, self.name, self.sigma, self.order, self.weight)
        if self.affine == 'affine':
            xx = x - self.center
    ...
    def applyDiffKmat(self, x, beta, firstVar=None):
        if firstVar is None:
            z = ku.applykdiffmat(x, x, beta, self.name, self.sigma, self.order, self.weight)
        else:
            z = ku.applykdiffmat(firstVar, x, beta, self.name, self.sigma, self.order, self.weight)
        return z

    # Computes A(i) = sum_j D_2[K(x(i), x(j))a2(j)]a1(j)
    def applyDiffKTensor(self, x, ay, ax, betay, betax, firstVar=None):
        if firstVar is None:
            z = ku.applydiffktensor(x, x, ay, ax, betay, betax, self.name, self.sigma, self.order, self.weight)
        else:
            z = ku.applydiffktensor(firstVar, x, ay, ax, betay, betax, self.name, self.sigma, self.order, self.weight)
        return z
    ...
    def applyDiffKT(self, x, p0, a, firstVar=None, regweight=1., lddmm=False, sym = False, extra_term = None, cpu=False):
        ...
        if self.localMaps:
            if self.localMaps[2] == 'naive':
                zpx = ku.applylocalk_naivedifft(y ,x , p ,a , self.name, self.sigma ,self.order, self.weight,
                                                regweight=regweight, lddmm=lddmm)
                #ku.applylocalk_naivedifft.parallel_diagnostics(level=4)
            else:
                zpx = ku.applylocalkdifft(y, x, p, a, self.name,  self.sigma, self.order, self.weight, self.localMaps[0],
                                           self.localMaps[1], regweight=regweight, lddmm=lddmm)
        else:
            zpx = ku.applyDiffKT(y ,x , p ,a , self.name, self.sigma ,self.order, self.weight,
                                 regweight=regweight, lddmm=lddmm, sym=sym, cpu=cpu, dtype=self.pk_dtype)
        if self.affine == 'affine':
            xx = x-self.center
    ...
    def applyDivergence(self, x,a, firstVar=None):
        ...
        if self.localMaps:
            if self.localMaps[2] == 'naive':
                zpx = ku.applylocalk_naivediv(y, x,a, self.name, self.sigma, self.order, self.weight)
            else:
                zpx = ku.applylocalkdiv(y, x, a, self.name, self.sigma, self.order, self.weight, self.localMaps[0],
                                        self.localMaps[1])
        else:
            zpx = ku.applyDiv(y,x,a, self.name, self.sigma, self.order, self.weight)
    ...
    def applyDDiffK11and12(self, x, n, a, p):
        z = ku.applykdiff11and12(x, n, a, p, self.name, self.sigma, self.order, self.weight)
        if self.affine == 'affine':
            z += self.w1 * a @ (a.T @ p)
        elif self.affine == 'euclidean':
            for E in self.affine_basis:
