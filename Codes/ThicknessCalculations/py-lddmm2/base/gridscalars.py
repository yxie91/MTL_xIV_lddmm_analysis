...
class GridScalars:
    ...
    def saveImg(self, filename, normalize=False):
        saveImage(self.data, filename, normalize=normalize, origin=self.origin,
                  resol=self.resol)
    ...
    def readVTK(self, filename):
        if gotVTK:
            ...
            self.data = np.ndarray(shape=dim, order='F',
                                   buffer=v.GetPointData().GetScalars())
            self.dim = 3
        else:
...
def saveImage(img, filename, normalize=False, origin=(0, 0, 0),
              resol=(1., 1., 1.)):
    if len(img.shape) == 2:
        if normalize:
