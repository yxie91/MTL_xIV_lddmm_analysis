    ...
    def endOfIteration(self):
        ...
        for k in range(self.nsurf):
            ...
            for kk in range(self.Tsize+1):
                ...
                vf = vtkFields()
                vf.scalars['Jacobian'] = np.exp(Jt[-1][kk, nn:nn+n1])
                vf.scalars['Jacobian_T'] = AV[:, 0]
                vf.scalars['Jacobian_N'] = np.exp(Jt[-1][kk, nn:nn+n1])/(AV[:, 0])
                ...
                AV = (AV[0]/AV0[0])
                vf = vtkFields()
                vf.scalars['Jacobian'] = np.exp(Jt[k][kk, :])
                vf.scalars['Jacobian_T'] = AV[:, 0]
                vf.scalars['Jacobian_N'] = np.exp(Jt[k][kk, :])/AV[:,0]
