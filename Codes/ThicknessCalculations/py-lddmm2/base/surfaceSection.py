...
class SurfaceSection:
    ...
    def ComputeIntersection(self, surf, hyperplane, plot = None):
        if surf.edges is None:
            ...
            surf.getEdges()
            #print('ending edges')
        F, V, W = ComputeIntersection_(surf.vertices, surf.surfel, surf.edges, surf.faceEdges, surf.vertex_weights,
...
def Surf2SecGrad(surf, s, curveDistGrad, target_label = None, target_comp_info=None):
    ...
    if s0.curve.faces.shape[0] > 0:
        ...
        grad_ = CurveGrad2Surf(cgrad, cgradw, surf_.vertices, surf_.edges, surf_.vertex_weights, s.hyperplane.u, s.hyperplane.offset)
    else:
