...
#     import pygalmesh
# except:
#     print('Module pygalmesh not installed')
from ngsolve import Mesh
from netgen.csg import CSGeometry, Sphere as Sphere_ng, Plane as Plane_ng, Pnt, Vec
...
class Disc(Surface):
    def __init__(self, outerRes = 50, nrings = 4):
        super().__init__()
        x = []
        y = []
        ...
        DT = Delaunay(pts[:, :-1])
        self.vertices = pts
        self.faces = DT.simplices
        print(pts.shape, self.faces.max())
...
class FlatEllipse(Disc):
    def __init__(self, center = (0,0), a=1., b=1., outerRes = 50, nrings = 4):
        super().__init__(outerRes=outerRes, nrings = nrings)
        pts = np.zeros(self.vertices.shape)
        pts[:, 0] = center[0] + a * self.vertices[:,0]
        pts[:, 1] = center[1] + b * self.vertices[:,1]
        self.updateVertices(pts)
...
def sphere_netgen(maxh=0.01):
    geo = CSGeometry()
    geo.Add(Sphere_ng(Pnt(0, 0, 0), 1))
    ...
    return p, m['nodes'][:,:3]-1


class Pancake(Surface):
    def __init__(self, w=(1.,1.,1.), maxh=0.01, targetSize=1000):
        super().__init__()
        geo = CSGeometry()
        s1 = Sphere_ng(Pnt(1., 1., 1.), 1)
        p1 = Plane_ng(Pnt(1,0,0), Vec(1,0,0))
        p2 = Plane_ng(Pnt(-1,0,0), Vec(-1,0,0))
        geo.Add(s1*p1*p2)
        #mesh = Mesh(geo.GenerateMesh(maxh=maxh, perfstepsend=MeshingStep.MESHSURFACE))
        mesh = Mesh(geo.GenerateMesh(maxh=maxh))
        self.from_netgen(mesh, targetSize)
        self.vertices[:,0] *= np.sqrt(w[0])
        self.vertices[:,1] *= np.sqrt(w[1])
        self.vertices[:,2] *= np.sqrt(w[2])
        self.updateVertices(self.vertices)



class Sphere_pygal(Surface):
    def __init__(self, center=(0,0,0), radius=1, resolution=100, targetSize=1000, maxh=0.01):
        super().__init__()
        # print(m["nodes"][:, :3])
        #p,m = sphere_netgen(maxh=maxh)

        mesh = pv.Sphere(radius=radius, center=center)
        self.from_pv(mesh, targetSize=targetSize)
        # self.vertices = np.array(mesh.points)
        # self.faces = np.array(mesh.faces, dtype=int)
        # self.component = np.zeros(self.faces.shape[0], dtype=int)
        # self.vertex_weights = np.ones(self.vertices.shape[0], dtype=int)
        # self.face_weights = np.ones(self.faces.shape[0], dtype=int)
        # self.computeCentersAreas()
...
class Ellipse_pygal(Surface):
    def __init__(self, center=(0,0,0), I=None, resolution=100, targetSize=1000, maxh=0.01):
        super().__init__()
        # d = Ball_pygal_()
        # mesh = pygalmesh.generate_surface_mesh(d, max_facet_distance=0.01, min_facet_angle=30.0,
        #                                        max_radius_surface_delaunay_ball=0.05)
        p,m = sphere_netgen(maxh=maxh)
        self.vertices = np.array(center) + p @ sqrtm(I)
        self.faces = np.int_(m)
        self.component = np.zeros(self.faces.shape[0], dtype=int)
        self.vertex_weights = np.ones(self.vertices.shape[0], dtype=int)
        self.face_weights = np.ones(self.faces.shape[0], dtype=int)
        self.computeCentersAreas()
...
class Ellipse(Surface):
    def __init__(self, center=(0,0,0), radius=(1,1,1), rotation=None, resolution = 100, targetSize = 1000,
        ...
        self.updateVertices(v)
        self.lmk = np.array([[self.radius[0], 0, 0], [-self.radius[0], 0, 0],
                             [0, self.radius[1], 0], [0, -self.radius[1], 0],
                             [0, 0, self.radius[2]], [0,0,-self.radius[2]]])
