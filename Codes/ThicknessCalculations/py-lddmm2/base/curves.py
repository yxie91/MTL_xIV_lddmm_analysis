...
def normGrad___(x, v, faces, component):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
    linel = x2 - x1
    a = np.sqrt((linel**2).sum(axis=1))
    #print min(a)
...
def computeUnitFaceNormals__(faces, linel):
    a = np.sqrt((linel**2).sum(axis=1))
...
def computeUnitVertexNormals__(x, faces, linel):
    a = np.sqrt((linel**2).sum(axis=1))
    tau = linel / np.maximum(a[:, None], 1e-10)
    normals = np.zeros(x.shape)
    counts = np.zeros(x.shape[0], dtype=int)
    for i in range(faces.shape[0]):
        normals[faces[i,0], :] += tau[i,:]
        normals[faces[i,1], :] -= tau[i,:]
        counts[faces[i,0]] += 1
        counts[faces[i,1]] += 1
    a = np.sqrt((normals**2).sum(axis=1))
    normals /= np.maximum(a[:, None], 1e-10)
...
def normGrad(x, v, faces, component, weight=1.0):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
...
def normGradInvariant(x, v, faces, component):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
    linel = x2 - x1
    a = np.sqrt((linel**2).sum(axis=1))
    nl = computeUnitFaceNormals__(faces, linel)
    res = (((phi2-phi1)**2).sum(axis=1)/a).sum()
    nc = 1+component.max()
    #print fv.faces.shape, phi.shape, nl.shape
    for comp in range(nc):
        I = np.nonzero(component==comp)[0]
        c = ((phi2[I,:] - phi1[I,:])*nl[I,:]).sum()
        res -= (c**2)/a[I].sum()
    return res

def normGradInvariant3D(x, v, faces, component):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
    linel = x2 - x1
    a = np.sqrt((linel**2).sum(axis=1))
    tl = linel/a[...,np.newaxis]
    res = (((phi2-phi1)**2).sum(axis=1)/a).sum()
    nc = 1+component.max()
    for comp in range(nc):
        I = np.nonzero(component==comp)[0]
        M = ((a[I].sum()+1e-6)*np.eye(3) 
            - (linel[I,:,np.newaxis]*tl[I,np.newaxis,:]).sum(axis=0))
        c = np.cross(phi2[I,:]-phi1[I,:], tl[I,:]).sum(axis=0)
        res -= (np.linalg.solve(M,c)*c).sum()
    #print res
    return res

def h1AlphaNorm(x, v, faces, component):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    dphi = phi2-phi1
    x1 = x[faces[:, 0], :]
...
def h1AlphaNormInvariant(x, v, faces, component):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    dphi = phi2-phi1
    x1 = x[faces[:, 0], :]
...
def diffNormGradInvariant(x, v, faces, component, variables='both'):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
    linel = x2 - x1
    a2 = (linel**2).sum(axis=1)
    a = np.sqrt(a2)
    tl = linel/a[...,np.newaxis]
    nl = np.zeros(tl.shape)
    nl[:,0] = -tl[:,1]
...
def diffNormGradInvariant3D(x, v, faces, component, variables='both'):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
    ...
    - (linel[:,:,np.newaxis]*tl[:,np.newaxis,:]).sum(axis=0))
...
def diffNormGrad(x, v, faces, component, variables='both', weight=1.):
    gradphi = None
    gradx = None
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
...
def diffH1Alpha(x, v, faces, component, variables='both'):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
...
def diffH1AlphaInvariant(x, v, faces, component, variables='both'):
    phi1 = v[faces[:,0],:]
    phi2 = v[faces[:,1],:]
    x1 = x[faces[:, 0], :]
    x2 = x[faces[:, 1], :]
