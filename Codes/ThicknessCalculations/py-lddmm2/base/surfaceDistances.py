...
def varifoldNorm0_pykeops(fv1, KparDist, dtype = 'float64'):
    ...
    res = 0
    for js, s in enumerate(KparDist.sigma):
        Kij = ku.makeKij(y/s, x/s, KparDist.name, KparDist.order)
        res += (Kij* multiplier).sum(1).sum(0)[0] * KparDist.weight[js]
    return res
...
def varifoldNormDef_pykeops(fvDef, fv1, KparDist, dtype='float64'):
    ...
    for js, s in enumerate(KparDist.sigma):
        ...
        res += (Kijyy* multiplieryy).sum(1).sum(0)[0] * KparDist.weight[js] \
               - 2* (Kijyx* multiplieryx).sum(1).sum(0)[0] * KparDist.weight[js]
    return res
...
#    beta2 = a1w[:, np.newaxis] * a2w[np.newaxis, :] * f0(cr1cr2)
    ...
    for js, s in enumerate(KparDist.sigma):
        ...
        z1 += (Kijyy * u1).sum(1) * KparDist.weight - (Kijyx * u2).sum(1) * KparDist.weight
...
#                                        - KparDist.applyK(c2, u2, firstVar=c1, matrixWeights=True))
    ...
    for js, s in enumerate(KparDist.sigma):
        ...
        dz1 += (dKijyy * beta1).sum(1) * KparDist.weight / s - (dKijyx * beta2).sum(1) * KparDist.weight / s
    dz1 *= (2/3)
...
#        beta2 = f0(cr1cr2) * a1[:, np.newaxis] * a2w[np.newaxis, :]
        ...
        for js, s in enumerate(KparDist.sigma):
            Kijyy = ku.makeKij(c1i/s, c1j/s, KparDist.name, KparDist.order)
            Kijyx = ku.makeKij(c1i/s, c2j/s, KparDist.name, KparDist.order)
            z1w += (Kijyy * beta1).sum(1) * KparDist.weight - (Kijyx * beta2).sum(1) * KparDist.weight
