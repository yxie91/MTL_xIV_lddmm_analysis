...
def assignLabel(imgs,names,vtkFile,orig=0):
    '''
    Assume vtk coordinates based on origin at corner, if origin < 0, then assume origin in center of image
    vtk = based off of img hdr 
    '''
    ...
    #polydata = reader.GetOutput()    
    reader2 = vtk.vtkPolyDataReader()
    #reader2.SetFileVersion()
    reader2.SetFileName(vtkFile)
    reader2.ReadAllScalarsOn()
    reader2.ReadAllVectorsOn()
    reader2.Update()
    polydata2 = reader2.GetOutput()
    ...
    array = points.GetData()
    coords = vtk_to_numpy(array) # coords in XYZ?
    labsFinal = np.zeros((coords.shape[0],1))
    labsMaxVal = np.zeros((coords.shape[0],1))


    
    for j in range(len(imgs)):
        im = nib.load(imgs[j])
        imData = np.asanyarray(im.dataobj)
        print("imaging data shape")
        print(imData.shape)
        x0 = np.arange(imData.shape[0])*np.abs(im.affine[0,0])
        x1 = np.arange(imData.shape[1])*np.abs(im.affine[1,1])
        x2 = np.arange(imData.shape[2])*np.abs(im.affine[2,2])
        if (orig < 0):
            x0 = x0 - np.mean(x0)
            x1 = x1 - np.mean(x1)
            x2 = x2 - np.mean(x2)
        # confirm spacing directions 
        print("confirming spacing for " + names[j])
        x0t = np.argwhere(imData > 0)
        print(str(np.min(x0t[:,0])) + " to " + str(np.max(x0t[:,0])))
        print(str(np.min(x0t[:,1])) + " to " + str(np.max(x0t[:,1])))
        print(str(np.min(x0t[:,2])) + " to " + str(np.max(x0t[:,2])))
        
        print("vtk is ")
        print(str(np.min(coords[...,0])) + " to " + str(np.max(coords[...,0])))
        print(str(np.min(coords[...,1])) + " to " + str(np.max(coords[...,1])))
        print(str(np.min(coords[...,2])) + " to " + str(np.max(coords[...,2])))
        
        coordsMix = np.copy(coords)
        coordsMix[...,0] = coords[...,1]
        coordsMix[...,1] = coords[...,0]
        labs = ds.applyFunction(coordsMix, imData, x0, x1, x2, NN=0)
        print("lab values")
        print(np.unique(labs))
        labsFinal[labs > labsMaxVal] = j+1
        labsMaxVal = (labs > labsMaxVal)*labs + (labs <= labsMaxVal)*labsMaxVal
    
    for j in range(len(imgs)):
        array = vtk.vtkIntArray()
        array.SetNumberOfComponents(0)
        array.SetName(names[j])
        for i in range(polydata2.GetNumberOfPoints()):
            array.InsertNextValue(int(labsFinal[i] == j+1))
        polydata2.GetPointData().AddArray(array);
        
    writer = vtk.vtkPolyDataWriter()
    #writer = vtk.vtkBYUWriter()
    writer.SetFileName(vtkFile.replace(".vtk","_labels.vtk"))
    writer.SetInputData(polydata2)
    writer.Update()
    writer.Write()
    dictToSave = dict()
    dictToSave['labels'] = labsFinal
    dictToSave['labelNames'] = names
    sp.io.savemat(vtkFile.replace(".vtk","_labels.mat"),dictToSave)
