...
class BarSeqCellLoader:
    ...
    def __init__(self,rootDir,res,geneFeat='nu_G',dimEff=None,numF=None,deltaF=False,featNames=None,suff=None):
        '''
        ...
        if ('.pkl' in rootDir):
            ...
            self.filenames = glob.glob(rootDir + '*cellSlice*' + suff + '.npz') # aligned cells with regenerated whole
            newlist = []
...
if __name__ == '__main__':
    # Example: python3 load_BarSeqHalfBrainCells.py 'D076_1L' '_genes'
    dataDir = sys.argv[1]
    if len(sys.argv) > 2:
        feat = sys.argv[2] 
    else:
        feat = ''
    fp = '/cis/home/kstouff4/Documents/MeshRegistration/ParticleLDDMMQP/sandbox/SliceToSlice/BarSeq/HalfBrains/' + dataDir + '/0.25/'
    sigma = 0.2                    
    # Selection of All Features
    origDataFP = '/cis/home/kstouff4/Documents/SpatialTranscriptomics/BarSeq/HalfBrains/' + dataDir + '/'
    pref = 'filt_neurons_' + dataDir + '_CCFv2_newtypes'
    ...
    if 'gene' in feat:
        rt = sp.io.loadmat(origDataFP + pref + '_geneTypes.mat',appendmat=False)
        regionTypeNames = rt['geneNames']
        
    for i in range(len(regionTypeNames)):
        rtList.append(regionTypeNames[i][0][0])
    regionTypeNames = rtList
    
    a = BarSeqCellLoader(fp,[0.0,0.0,0.200],featNames=regionTypeNames,geneFeat='nu_S',dimEff=2,suff=feat)
    print("filenames are: ", a.filenames)
    print("feature names are:", a.featNames)
    particles,features = a.getSizes()
    a.saveToPKL(fp+'sig' + str(sigma) + '_initialHighAllCells_nu_R.pkl')
    a.writeAll(fp+'sig' + str(sigma) + '_initialHighAllCells_nu_R')
    #print(a.sizes)
    #print(a.numFeatures)
    #print(sum(a.sizes))
    a.subSampleStratified(fp,sigma,alpha=0.75)
    #a.writeSubSampled()
    a.saveToPKL(fp+'sig' + str(sigma) + '_initialHighLowAllCells_nu_R.pkl')
    #a.centerAndScaleAll()
