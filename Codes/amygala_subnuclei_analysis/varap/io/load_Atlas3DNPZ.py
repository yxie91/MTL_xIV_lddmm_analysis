...
class Atlas3DNPZLoader:
    
    def __init__(self,rootDir,res,dimEff=None,numF=None,deltaF=False,featNames=None,fileIndex=1):
        '''
        ...
        if ('.pkl' in rootDir):
            with open(rootDir, 'rb') as f:  # Python 3: open(..., 'rb')
                self.filenames, self.res, self.sizes,self.numFeatures,self.deltaF,self.dimEff,self.filenames_subsample,self.featNames,self.fileIndex = pickle.load(f)
            ...
            self.res = res
            self.numFeatures = allFiles[allFiles.files[fileIndex]].shape[-1]
            self.deltaF = deltaF
            ...
            self.filenames = glob.glob(rootDir + '*mouseAtlas.npz')
    ...
    def getSizes(self):
        ...
        for f in self.filenames:
            ...
            if len(n[n.files[self.fileIndex]].shape) < 2 or n[n.files[self.fileIndex]].shape[1] == 1 or self.deltaF:
                uniqueF.append(np.unique(n[n.files[self.fileIndex]]))
            
            else:
                if self.numFeatures is None:
                    self.numFeatures = n[n.files[self.fileIndex]].shape[1]
                else:
                    print("Expected Features is ", self.numFeatures)
                    print("Features Read in Dataset is ", n[n.files[self.fileIndex]].shape[1])
        if self.numFeatures is None:
    ...
    def getSlice(self,index):
        ...
        features = info[info.files[self.fileIndex]]
    ...
    def saveToPKL(self,outpath):
        with open(outpath, 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump([self.filenames, self.res, self.sizes, self.numFeatures,self.deltaF,self.dimEff,self.filenames_subsample,self.featNames,self.fileIndex], f)
        return
...
if __name__ == '__main__':
    fp = '/cis/home/kstouff4/Documents/MeshRegistration/Particles/DevelopmentalAtlases/DeMBA_P4_KimLabDevCCFv001_330.npz'
    fpBase = '/cis/home/kstouff4/Documents/MeshRegistration/Particles/DevelopmentalAtlases/'
    a = Atlas3DNPZLoader(fp,[0.02,0.02,0.02],dimEff=3,fileIndex=1)
    particles,features = a.getSizes()
    a.saveToPKL(fp.replace('.npz','_initialHigh.pkl'))
    a.writeAll(fp.replace('.npz','_initialHigh'))
    sigma = 0.2
    a.subSampleStratified(fpBase,sigma,alpha=0.75)
    a.writeSubSampled()
    a.saveToPKL(fp.replace('..npz','_initialHighLow_sig0.2.pkl'))
    '''            
    fp = '/cis/home/kstouff4/Documents/MeshRegistration/Particles/ExVivoHuman11T/HighFieldSegs/brain' + bnum + '.npz'
    a = MRILoader(fp,[0.125,0.125,0.125],dimEff=3,fileIndex=1)
    print("filenames are: ", a.filenames)
    #a.featNames = genes
    print("feature names are:", a.featNames)
    particles,features = a.getSizes()
    #a.centerAndScaleAll()
    a.saveToPKL(fp.replace('brain' + bnum + '.npz','initialHighBrain' + bnum + '_3Regions.pkl'))
    a.writeAll(fp.replace('brain' + bnum + '.npz','initialHighBrain' + bnum + '_3Regions'))
    #print(a.sizes)
    #print(a.numFeatures)
    #print(sum(a.sizes))
    sigma = 0.500
    ...
    a.writeSubSampled()
    a.saveToPKL(fp.replace('brain' + bnum + '.npz','initialHighLowBrain' + bnum + '_3Regions.pkl'))
    '''
