...
def main(referencetype): 
    ...
    for i in range(1,3):
        ...
        '''for i, subj in enumerate(list(LH_Tau.index.values)):
            ax.annotate(subj, (LH_Tau.values[i], RH_Tau.values[i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=10)'''
    print(Xmin,Xmax)
    x_linspace = np.linspace(min(Xmin), max(Xmax), 50)#0,2.5,50)#
    ...
    ax.set_xlim((0.4, 2.6))
    ax.set_ylim((0.4, 2.6))
    ticks = np.arange(0.5, 2.51, 0.5)   # 0.4, 0.6, 0.8, …, 2.4, 2.6 (2.6 excluded)
    ax.set_xticks(ticks)
    ax.set_yticks(ticks)
    ...
    plt.tight_layout()
    outname = f'Figure/F15_BIOCARD_ERCHippo_Tau_consistency_{referencetype}.png'
