...
def main(referencetype): 
    DATA=pd.read_excel(f'Results/ADNI_MTLall_FUSE_{referencetype}_new.xlsx',index_col=0)
    LDATA=pd.read_excel(f'Results/ADNI_MTLall_LH_{referencetype}_new.xlsx',index_col=0)
    RDATA=pd.read_excel(f'Results/ADNI_MTLall_RH_{referencetype}_new.xlsx',index_col=0)
    LA_subjects=[]
    HA_subjects=[]
    ...
    LA_DATA=pd.concat([LDATA.loc[list(set(LA_subjects))],RDATA.loc[list(set(LA_subjects))]],axis=0)
    HA_DATA=pd.concat([LDATA.loc[list(set(HA_subjects))],RDATA.loc[list(set(HA_subjects))]],axis=0)
    ...
    for i in range(1,3):
        ...
        xr=xrange[r]
        HA_VA=HA_DATA[f"{r} VA"]
        LA_VA=LA_DATA[f"{r} VA"]
        VA=pd.concat([HA_VA,LA_VA],axis=0)
        #print(VA.shape)
        #print(HA_VA)
        HA_Tau=HA_DATA[f"{r} Tau"]
        LA_Tau=LA_DATA[f"{r} Tau"]
        Tau=pd.concat([HA_Tau,LA_Tau],axis=0)
        #print(HA_Tau)
        HA_Amyloid=HA_DATA[f"{r} Amyloid"]
        LA_Amyloid=LA_DATA[f"{r} Amyloid"]
        Amyloid=pd.concat([HA_Amyloid,LA_Amyloid],axis=0)

        fontsize = 22
        ax = axes
        ax.scatter(VA, Tau, alpha=0.85, color=COLOR_POS, edgecolor='black', s=80)
        '''for i, subj in enumerate(list(VA.index.values)):
            ax.annotate(subj, (VA.values[i], Tau.values[i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8)'''
        add_fit_ci_pi(ax, VA, Tau, color=COLOR_POS, label=None, lw=5, ci_alpha=0.6, pi_alpha=0)
        region_lines.append(Line2D([], [], linestyle="--", lw=5, color=COLOR_POS, label=r))
        ax.set_xlabel("Volume Atrophy Rate", fontsize=fontsize)
        ax.set_ylabel("Tau PET Accumulation", fontsize=fontsize)
        ax.set_xlim(xr); ax.set_ylim((0.4, 2.6))
        xticks = np.arange(-2, 12.1, 2)
        yticks = np.arange(0.5, 2.51, 0.5)
        ax.set_xticks(xticks)
        ax.set_yticks(yticks)
        xfmt = FuncFormatter(lambda x, _: f"{x:.0f}")
        yfmt = FuncFormatter(lambda x, _: f"{x:.1f}")
        ax.xaxis.set_major_formatter(xfmt)
        ax.yaxis.set_major_formatter(yfmt)
        ax.tick_params(axis='both', labelsize=18)
        ax.grid(True, alpha=0.2)
        #ax.set_aspect('equal', adjustable='box')
        # legend for HA: single color + its CI
        line_ha = Line2D([], [], linestyle="--", lw=5, color=COLOR_POS, label=f"{r}")
        ci_ha   = Patch(facecolor=COLOR_POS, edgecolor='black', alpha=0.6, label="95% CI")
        ax.legend(handles=[line_ha, ci_ha], fontsize=14, loc='upper left', frameon=True)
        plt.tight_layout()
        tag = f"ERCTEC" if r == "ERC/TEC" else f"{r}"
        outname = f'Figure/F15_ADNI34_{referencetype}_{tag}_TauVA_MCIDAT_new.png'
