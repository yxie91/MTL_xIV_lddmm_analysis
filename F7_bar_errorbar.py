...
# Regions (left y-axis)
ax.bar(x_control + offsets, BIOCARD_control_mean, width=bar_w, color=con_colors, edgecolor="black", linewidth=1.0, yerr=BIOCARD_control_std, capsize=5)
ax.bar(x_mci     + offsets, BIOCARD_mcidat_mean, width=bar_w, color=mci_colors, edgecolor="black", linewidth=1.0, yerr=BIOCARD_mcidat_std, capsize=5)
ax.set_ylim(-2.5,8)
# Ages (right y-axis)
'''ax_r = ax.twinx()
ax_r.set_ylim(65, 85)
...
plt.tight_layout()
plt.savefig(f"Figure/BIOCARD_VA_errorbar.png", dpi=300, bbox_inches="tight")
plt.close()